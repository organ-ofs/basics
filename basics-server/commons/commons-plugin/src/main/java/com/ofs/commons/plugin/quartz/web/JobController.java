package com.ofs.commons.plugin.quartz.web;

import com.ofs.commons.plugin.quartz.dto.JobTaskDto;
import com.ofs.commons.plugin.quartz.entity.JobTask;
import com.ofs.commons.plugin.quartz.service.impl.JobServiceImpl;
...
public class JobController {
    ...
    public String refresh(@PathVariable @NotNull String id) throws SchedulerException {
        String result;
        JobTask entity = jobService.getById(id);
    ...
    public String modifyJob(@RequestBody @Validated JobTaskDto dto) {
        ...
        synchronized (log) {
            JobTask job = jobService.getById(dto.getId());
            if (job.getStatus().equals(OPEN)) {
                try {
                    JobKey jobKey = jobService.getJobKey(job);
